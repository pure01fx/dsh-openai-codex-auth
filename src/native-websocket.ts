...
} from './native-websocket-socket.js'
...
interface SessionEntry {
  socket: NativeCodexWebSocket | undefined
  socketCredential: string | undefined
  protocol: NativeCodexWebSocketSessionState
  disabled: boolean
...
function boundedPositive(
  value: number | undefined, fallback: number, maximum: number, label: string,
): number {
  const resolved = positive(value, fallback, label)
  if (resolved > maximum) throw failure(`native Codex ${label} exceeds its maximum`, 'INVALID_ARGS')
  return resolved
}
...
function sessionKey(generation: GenerateOptions, routingId: string): string | undefined {
  if (generation.sessionId === undefined) return undefined
  return createHash('sha256').update(routingId).digest('base64url')
}

function turnKey(generation: GenerateOptions): string {
...
function normalizedOutputItem(event: ResponsesStreamEvent): Record<string, unknown> | undefined {
  if (event.type !== 'response.output_item.done' || event.item === undefined) return undefined
...
function eventHeader(event: Record<string, unknown>, name: string): string | undefined {
  if (typeof event.headers !== 'object' || event.headers === null) return undefined
  for (const [key, raw] of Object.entries(event.headers as Record<string, unknown>)) {
...
function eventErrorFacts(
  event: Record<string, unknown>, status: number | undefined,
...
function eventFailure(event: Record<string, unknown>): LlmError | undefined {
  ...
  if (status === 403) return new LlmError('native Codex WebSocket authorization failed', 'AUTH', facts)
...
export class NativeCodexWebSocketTransport implements NativeCodexTransport {
  private readonly http: NativeCodexHttpTransport
  private readonly factory: NativeCodexWebSocketFactory
  ...
  constructor(private readonly options: NativeCodexWebSocketTransportOptions) {
    this.http = new NativeCodexHttpTransport(options)
    this.factory = options.webSocketFactory ?? new NodeNativeCodexWebSocketFactory()
    this.connectTimeoutMs = boundedPositive(
      options.webSocketConnectTimeoutMs, 10_000, 120_000, 'WebSocket connect timeout',
    )
    ...
    this.sessionIdleMs = boundedPositive(
      options.webSocketSessionIdleMs, DEFAULT_SESSION_IDLE_MS, 24 * 60 * 60_000,
  ...
  private entry(key: string | undefined): SessionEntry {
    const now = Date.now()
  ...
  private assertFastAuthority(credential: NativeCodexCredential, mode: NativeCodexTransportMode): void {
  ...
  private headers(prepared: NativeCodexPreparedRequest, credential: NativeCodexCredential): Record<string, string> {
    return {
  ...
  private async ensureSocket(
    entry: SessionEntry,
    prepared: NativeCodexPreparedRequest,
    credential: NativeCodexCredential,
    signal: AbortSignal,
  ): Promise<void> {
    const fingerprint = socketCredential(credential)
    if (entry.socket !== undefined && entry.socketCredential === fingerprint) return
    this.closeEntry(entry)
  ...
  private async receive(entry: SessionEntry, signal?: AbortSignal): Promise<string> {
    if (entry.socket === undefined) throw failure('native Codex WebSocket is unavailable', 'WS_RETRYABLE')
  ...
  private async *exchange(
    ...
    while (true) {
      ...
      if (entry.turnState === undefined) {
        ...
        if (nextTurnState !== undefined) entry.turnState = nextTurnState
      }
      ...
      if (event.type === 'response.completed') {
        const response = typeof (event as unknown as Record<string, unknown>).response === 'object'
            && (event as unknown as Record<string, unknown>).response !== null
          ? (event as unknown as Record<string, unknown>).response as Record<string, unknown>
          : undefined
      ...
      if (!prewarm) for (const chunk of chunks) yield chunk
      if (translator.terminated) {
        if (event.type !== 'response.completed') entry.protocol.reset()
        return
      }
    }
  }
  ...
  async *stream(
    ...
    if (entry.busy) throw failure('native Codex WebSocket session already has an active request', 'CONCURRENT_REQUEST')
    ...
    const fallbackMode = (): NativeCodexTransportMode => ({
      ...mode,
    ...
    try {
      ...
      while (true) {
        let emitted = false
        let attemptedCredential: NativeCodexCredential | undefined
      ...
      if (!requestCompleted || !entry.pooled) this.closeEntry(entry)
    }
  }
