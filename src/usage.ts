...
import {
  parseCodexRateLimitCredits,
  type CodexRateLimitCredits,
  type CodexRateLimitUpdate,
} from './rate-limits.js'
...
function boundedUsageText(value: unknown): string | undefined {
  if (typeof value !== 'string') return undefined
  const text = value.trim()
  return text.length > 0 && text.length <= 128 ? text : undefined
}

function normalizedUsageLimitId(value: unknown): string | undefined {
...
function sortedUsageLimits(limits: Iterable<UsageLimitSummary>): UsageLimitSummary[] {
...
export function normalizeUsage(value: unknown): UsageSummary {
  ...
  if (Array.isArray(additional)) {
    for (const value of additional.slice(0, 31)) {
      ...
      if (id === undefined || id === 'codex' || limits.some(limit => limit.id === id)) continue
      const extra = usageLimit(
        id,
  ...
  const resetCreditRow = resetCreditValue !== null && typeof resetCreditValue === 'object'
    ? resetCreditValue as Record<string, unknown> : undefined
...
function mergeUsageLimit(
  previous: UsageLimitSummary | undefined,
  update: CodexRateLimitUpdate,
): UsageLimitSummary {
...
export function mergeDirectUsage(
