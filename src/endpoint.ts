...
function invalid(message: string, cause?: unknown): LlmError {
...
export function nativeCodexEndpoint(
  endpoint: string, allowWebSocketProtocols = false,
): URL {
  let url: URL
  try { url = new URL(endpoint) } catch (error) {
